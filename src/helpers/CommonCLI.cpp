...
static void setMQTTPrefsDefaults(MQTTPrefs* prefs) {
  ...
  prefs->mqtt_tx_enabled = 1;        // all TX packets enabled by default
  prefs->mqtt_rx_enabled = 1;        // RX packets enabled by default
  prefs->mqtt_status_interval = 300000; // 5 minutes default
  // Slot presets: dutchmeshcore collectors enabled by default, rest = none
    ...
    *dp = 0;
    ...
    if (memcmp(subcmd, "preset ", 7) == 0) {
      ...
      if (findMQTTPreset(preset_name) != nullptr ||
        ...
        strcpy(reply, "Error: valid presets are: dutchmeshcore-1, dutchmeshcore-2, analyzer-us, analyzer-eu, meshmapper, meshrank, waev, meshomatic, cascadiamesh, tennmesh, nashmesh, custom, none");
      }
